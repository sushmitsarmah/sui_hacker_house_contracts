...
module site_forge::suins_integration;
...
public struct SiteForgeApp has drop {}
...
public fun claim(
    siteforge_nft: &mut SubscriptionNFT,
    suins: &mut SuiNS,
    ...
    // Verify that the caller owns the siteforge_nft
    assert!(tx_context::sender(ctx) == sub_nft::owner(siteforge_nft), errors::not_nft_owner());

    // verify we can register names using this app.
    suins.assert_app_is_authorized<SiteForgeApp>();
    ...
    // activate the siteforge_nft if it's not activated.
    // This will grant it access to future promotions.

    let registry = suins::app_registry_mut<SiteForgeApp, Registry>(SiteForgeApp {}, suins);
